...
def process_one_las_multi_defects(las_path, output_dir):
    ...
    # 1. 读取原始数据
    raw_las_data = IOManager.load_las(las_path)
    
    # 2. 【新增】过滤操作：只保留 classification == 2 (通常是地面点) 的点
    # 注意：这里需要根据你使用的 IOManager 返回的对象类型来操作
    # 假设返回的是 laspy 对象，常用的过滤方法如下：
    mask = raw_las_data.classification == 2
    
    # 如果该文件里根本没有分类为 2 的点，直接跳过
    if np.sum(mask) == 0:
        return f"File: {file_name}.las - SKIPPED (No points with classification 2 found)"

    # 提取符合条件的点，创建一个新的点云数据副本
    las_data = raw_las_data[mask]
    
    # 3. 重置分类标签（只针对过滤后的点进行操作）
    ...
    file_log_content = [f"File: {file_name}.las (Filtered: only Class 2 points kept)"]
    
    defect_count = 0
    current_direction = None 

    init_args = Args()
    file_log_content.append(f"   [Base Args] NoiseStd: {init_args.noise_std}, SlopeStatus: {init_args.slope_status}")
    
    # 初始化编辑器（此时编辑器操作的是过滤后的点）
    editor = LASVisualEditor(las_data, init_args)
    editor.apply_noise()

    while True:
        current_ratio = get_current_defect_ratio(las_data)
        if current_ratio >= TARGET_RATIO or defect_count >= MAX_TOTAL_DEFECTS:
            break
    ...
    # 保存文件
    out_las_path = os.path.join(output_dir, f"{file_name}_ground_only.las")
